...
Transaction.prototype.commit = function( client, cb ){
  ...
  client.bulk( { body: payload }, function( err, resp ){
    ...
    if( this.batch._status > status.CREATED ){
      console.log( 'retrying batch', '[' + this.batch._status + ']' );
      this.batch.retries++;
...
Transaction.updateBatch = function( batch, items ){
  // console.log( resp.items.length, batch._operations.length, payload.length );
  items.forEach( function( item, i ){
    ...
    // console.log( 'set operation status', operation.status, JSON.stringify( res, null, 2 ) );
